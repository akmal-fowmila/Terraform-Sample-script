resource "pingfederate_idp_adapter" "html_form_adapter" {
  adapter_id = var.adapter_id
  ...
  # Ensure the PCV is created *before* the Adapter
  depends_on = [
    pingfederate_password_credential_validator.simple_pcv
  ]
}
}